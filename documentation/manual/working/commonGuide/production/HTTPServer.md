...
Only add identifiers that are generated by trusted infrastructure and cannot be supplied by clients. The setting matches identifiers exactly, only applies to RFC 7239 `Forwarded` headers, and does not make the `unknown` identifier trusted.
